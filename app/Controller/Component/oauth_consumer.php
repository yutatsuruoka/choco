...
class OauthConsumerComponent extends Component {
    ...
    private function createConsumer($consumerName) {
        if (!class_exists("AbstractConsumer", false)) {
            $CONSUMERS_PATH = dirname(__FILE__) . DS . 'oauth_consumers' . DS;
            if (!App::import('File', 'AbstractConsumer', array('file'
                        => $CONSUMERS_PATH . 'abstract_consumer.php'))) {
                throw new InvalidArgumentException(' abstractConsumernot found!');
            }
        }
        ...
        if (!class_exists($className, false)) {
            if (App::import('File', $className, array('file' => $CONSUMERS_PATH . $fileName))) {
                $consumerClass = new $className();
                return $consumerClass->getConsumer();
            } else {
                throw new InvalidArgumentException('Consumer ' . $CONSUMERS_PATH . $fileName . ' not found!');
            }
        }
    }
    ...
    private function getPathToVendorsFolderWithOAuthLibrary() {
        ...
        if ($pathToVendorsFolder == '') {
            ...
            } elseif (file_exists(APP . 'Vendor' . DS . 'OAuth')) {
